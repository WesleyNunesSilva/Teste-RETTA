...
class SyncDeputyExpensesFromApi implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($deputies as $deputy) {
            $response = Http::get("https://dadosabertos.camara.leg.br/api/v2/deputados/{$deputy->external_id}/despesas", [
                'itens' => 100, // você pode ajustar conforme necessário
                'ordem' => 'DESC',
            ...
            foreach ($expenses as $data) {
                if (!isset($data['codDocumento'])) {
                    Log::warning("Despesa ignorada: sem codDocumento", ['data' => $data]);
                    continue;
                }
                ...
                Expense::updateOrCreate(
                    ...
                    [
                        ...
                        'amount' => $data['valorLiquido'] ?? 0,
                        'supplier_name' => $data['nomeFornecedor'] ?? null,
                    ]
                );
            }

            Log::info("Despesas do deputado {$deputy->name} sincronizadas com sucesso.");
        }
    }
}
*/